...
class OfflineCachedProfileScreenTest {
    ...
    fun showsOfflineMessage_whenProfileIsNull() {
        composeRule.setContent {
            MaterialTheme {
                OfflineCachedProfileScreen(profile = null)
            }
        }
    ...
    fun showsProfileAndNoHuntsMessage_whenProfileHasNoHunts() {
        ...
        composeRule.setContent {
            MaterialTheme {
                OfflineCachedProfileScreen(profile = profile)
            }
        }
    ...
    fun switchesTabs_andShowsCorrespondingHunts() {
        ...
        val profile =
            sampleProfile(
                ...
                bio = "Some bio",
                myHunts = listOf(
                    sampleHunt(uid = "my1", title = myHuntTitle)
                ),
                doneHunts = listOf(
                    sampleHunt(uid = "done1", title = doneHuntTitle)
                ),
                likedHunts = listOf(
                    sampleHunt(uid = "liked1", title = likedHuntTitle)
                )
            )
        ...
        composeRule.onNodeWithContentDescription(OfflineProfileConstants.TAB_DONE_HUNTS)
            .performClick()
        composeRule.onNodeWithText(doneHuntTitle).assertExists()

        composeRule.onNodeWithContentDescription(OfflineProfileConstants.TAB_LIKED_HUNTS)
            .performClick()
