...
class HuntsRepositoryFirestore(
  ...
  override suspend fun getAllMyHunts(authorID: String): List<Hunt> {
    val snapshot =
        db.collection(HUNTS_COLLECTION_PATH).whereEqualTo(HuntsRepositoryFirestoreConstantsStrings.AUTHOR_ID, authorID).get().await()
    return snapshot.mapNotNull { documentToHunt(it) }
  }
  ...
  override suspend fun getHunt(huntID: String): Hunt {
    ...
    return documentToHunt(document)
        ?: throw IllegalArgumentException("${HuntsRepositoryFirestoreConstantsStrings.HUNT} $huntID ${HuntsRepositoryFirestoreConstantsStrings.NOT_FOUND}")
  }
  ...
  override suspend fun addHunt(hunt: Hunt, mainImageUri: Uri?, otherImageUris: List<Uri>) {
    ...
    try {
      ...
      Log.e(HuntsRepositoryFirestoreConstantsStrings.TAG, "${HuntsRepositoryFirestoreConstantsStrings.ERROR_IMAGE_UPLOADING} ${hunt.uid}", e)

      // Optionally clean up any uploaded images if partial success occurred
      try {
        imageRepo.deleteAllHuntImages(hunt.uid)
      } catch (cleanupError: Exception) {
        Log.w(HuntsRepositoryFirestoreConstantsStrings.TAG,
            HuntsRepositoryFirestoreConstantsStrings.ERROR_CLEANUP, cleanupError)
      }
  ...
  private fun documentToHunt(document: DocumentSnapshot): Hunt? {
    return try {
      ...
      val start =
          startData?.let {
            Location(
                ...
                longitude = it[HuntsRepositoryFirestoreConstantsStrings.LONGITUDE] as? Double ?: ZERO,
                name = it[HuntsRepositoryFirestoreConstantsStrings.NAME] as? String ?:HuntsRepositoryFirestoreConstantsStrings.EMPTY,
                description = it[HuntsRepositoryFirestoreConstantsStrings.DESCRIPTION] as? String ?:HuntsRepositoryFirestoreConstantsStrings.EMPTY,
                imageIndex = (it[HuntsRepositoryFirestoreConstantsStrings.IMAGE_INDEX] as? Long)?.toInt())
      ...
      val middlePointsData = document.get(HuntsRepositoryFirestoreConstantsStrings.MIIDDLE_POINT) as? List<Map<*, *>>
      val middlePoints =
      ...
      val statusString = document.getString(HuntsRepositoryFirestoreConstantsStrings.STATUS) ?: return null
      ...
      val description = document.getString(HuntsRepositoryFirestoreConstantsStrings.DESCRIPTION) ?: return null
      val time = document.getDouble(HuntsRepositoryFirestoreConstantsStrings.TIME) ?: return null
      val distance = document.getDouble(HuntsRepositoryFirestoreConstantsStrings.DISTANCE) ?: return null
      val difficultyString = document.getString(HuntsRepositoryFirestoreConstantsStrings.DIFFICULTY) ?: return null
      val difficulty = Difficulty.valueOf(difficultyString)
      val authorId = document.getString(HuntsRepositoryFirestoreConstantsStrings.AUTHOR_ID) ?: return null
      val mainImageUrl = document.getString(HuntsRepositoryFirestoreConstantsStrings.MAIN_IMAGE) ?: HuntsRepositoryFirestoreConstantsStrings.EMPTY
      val otherImagesUrls =
          (document.get(HuntsRepositoryFirestoreConstantsStrings.OTHER_IMAGE) as? List<*>)?.filterIsInstance<String>() ?: emptyList()
      val reviewRate = document.getDouble(HuntsRepositoryFirestoreConstantsStrings.RATING_REVIEW) ?: return null
      ...
      Log.e(HuntsRepositoryFirestoreConstantsStrings.TAG, HuntsRepositoryFirestoreConstantsStrings.ERROR_CONVERTING, e)
      null
    }
  }
}
