...
object NotificationTokenService {
     ...
     *
     * The token is stored in the user's profile document using merge semantics,
     * ensuring that existing profile fields are not overwritten.
     *
     * Storage location:
     * - Collection: profiles
     * - Document: [userId]
     ...
     *
     * This function does not perform authentication checks and assumes the caller
     * has already validated the user identity.
     *
  ...
  fun persistToken(userId: String, token: String): Task<Void> {
    ...
    /**
     * Fetches the current device's FCM registration token and persists it
     * to the given user's profile document.
     *
     * This function suspends until the token is retrieved from Firebase
     * Messaging and the Firestore write completes.
     *
     * Intended to be called after a successful sign-in or when restoring
     * an authenticated session to ensure the backend always has the latest
